...
class ChatTemplateParser:
    ...
    def tokenize_and_mask(self, messages):
        if torch is None:
            raise RuntimeError("torch is not available; tokenize_and_mask is unsupported in Memex.")
        try:
...
class QwenChatTemplateParser(ChatTemplateParser):
    def __init__(self, tokenizer, disable_thinking=False):
        ...
        if disable_thinking:
            # Empty thinking block when thinking is disabled
            self.generation_prompt = self.assistant_token + "<think>\n\n</think>\n\n"
        else:
            # Match vLLM's chat_template: add_generation_prompt includes <think>\n
            self.generation_prompt = self.assistant_token + "<think>\n"
    ...
    def parse_completion(self, completion_ids):
        ...
        tool_calls = self.tool_parser.parse_with_errors(content).tool_calls
